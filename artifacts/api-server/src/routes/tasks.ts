...
function computeStatus(task: { completed: boolean; deadlineDate: Date | null }): "pending" | "completed" | "overdue" {
  if (task.completed) return "completed";
  if (task.deadlineDate && new Date() > task.deadlineDate) return "overdue";
  return "pending";
}
...
router.put("/tasks/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  if (parsed.data.deadlineDate !== undefined) updates.deadlineDate = parsed.data.deadlineDate ? new Date(parsed.data.deadlineDate) : null;
  if (parsed.data.reminderTime !== undefined) updates.reminderTime = parsed.data.reminderTime ? new Date(parsed.data.reminderTime) : null;
  ...
  const mergedDeadline = (parsed.data.deadlineDate !== undefined
    ? (parsed.data.deadlineDate ? new Date(parsed.data.deadlineDate) : null)
...
router.patch("/tasks/:id/complete", requireAuth, async (req, res): Promise<void> => {
  ...
  const [task] = await db.update(tasksTable)
    .set({ completed: newCompleted, status: newStatus })
    .where(and(eq(tasksTable.id, id), eq(tasksTable.userId, req.user!.userId)))
    .returning();
